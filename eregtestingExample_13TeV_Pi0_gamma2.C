...
void eregtestingExample_13TeV_Pi0_gamma2(bool dobarrel=true, bool doele=false) {
  
  //output dir
  
  TString dirname = "eregexampletest_test_gamma2/EE";//"/data/bendavid/eregexampletest/eregexampletest_test/"; 
  if(dobarrel)
	{
	dirname = "eregexampletest_test_gamma2/EB";
	}
  ...
  TString infile = TString::Format("../../eregexampletest_gamma2/%s",fname.Data());
  ...
  if (doele) {
    ...
    if(dobarrel)
    {
    TFile *fdin = TFile::Open("/afs/cern.ch/work/z/zhicaiz/public/ECALpro_MC_TreeForRegression/sum_Pi0Gun_Flat0to50bx25_EB.root");//("root://eoscms.cern.ch///eos/cms/store/cmst3/user/bendavid/idTreesAug1/hgg-2013Final8TeV_ID_s12-h124gg-gf-v7n_noskim.root");
   // TDirectory *ddir = (TDirectory*)fdin->FindObjectAny("PhotonTreeWriterPreselNoSmear");
    dtree = (TTree*)fdin->Get("Tree_Optim");
    }      
   else
    {
  TFile *fdin = TFile::Open("/afs/cern.ch/work/z/zhicaiz/public/ECALpro_MC_TreeForRegression/sum_Pi0Gun_Flat0to50bx25_EE.root");//("root://eoscms.cern.ch///eos/cms/store/cmst3/user/bendavid/idTreesAug1/hgg-2013Final8TeV_ID_s12-h124gg-gf-v7n_noskim.root");
   // TDirectory *ddir = (TDirectory*)fdin->FindObjectAny("PhotonTreeWriterPreselNoSmear");
    dtree = (TTree*)fdin->Get("Tree_Optim");
  ...
  TCut selcut = "(STr2_enG2_true/cosh(STr2_Eta_2)>1.0) && (STr2_S4S9_2>0.75)";
/*  
TCut selcut;
  if (dobarrel) 
