...
void eregtesting_13TeV_Pi0_newCut(bool dobarrel=true, bool doele=false,int gammaID=0) {
  ...
  TString infile = TString::Format("../../Spring16TrainSpring16Test/ereg_ws/%s/%s",gammaDir.Data(),fname.Data());
  ...
  TString infile2 = TString::Format("../../ereg_ws/%s/%s",gammaDir.Data(),fname.Data());
  TFile *fws2 = TFile::Open(infile2); 
  RooWorkspace *ws2 = (RooWorkspace*)fws2->Get("wereg");
  //read variables from workspace
  RooGBRTargetFlex *meantgt2 = static_cast<RooGBRTargetFlex*>(ws2->arg("sigmeant"));  
  RooRealVar *tgtvar2 = ws2->var("tgtvar");
  RooArgList vars2;
  vars2.add(meantgt2->FuncVars());
  vars2.add(*tgtvar2);
  ...
  TCut selcut2 = "(STr2_enG_nocor/cosh(STr2_Eta)>1.0) && (STr2_S4S9 > 0.75) && (STr2_isMerging < 2) && (STr2_DeltaR < 0.03) && (STr2_ptPi0_nocor>3.0) && (STr2_DeltaRG1G2<0.1) && (STr2_enG_nocor>1.0)";
  //TCut selcut2 = "(STr2_enG_nocor/cosh(STr2_Eta)>1.0) && (STr2_S4S9 > 0.75) && (STr2_isMerging < 2) && (STr2_DeltaR < 0.03) && (STr2_mPi0_nocor>0.1)";
  ...
  RooRealVar weightvar2("weightvar2","",1.);
  weightvar.SetTitle(selcut);
  weightvar2.SetTitle(selcut2);
  
  //make testing dataset
  RooDataSet *hdata = RooTreeConvert::CreateDataSet("hdata",dtree,vars,weightvar);   
  RooDataSet *hdata2 = RooTreeConvert::CreateDataSet("hdata",dtree,vars2,weightvar2);   
  ...
  //retrieve full pdf from workspace
  RooAbsPdf *sigpdf2 = ws2->pdf("sigpdf");
  //regressed output functions
  RooAbsReal *sigmeanlim2 = ws2->function("sigmeanlim");
  RooAbsReal *sigwidthlim2 = ws2->function("sigwidthlim");
  RooAbsReal *signlim2 = ws2->function("signlim");
  RooAbsReal *sign2lim2 = ws2->function("sign2lim");
  ...
  //formula for corrected energy/true energy ( 1.0/(etrue/eraw) * regression mean)
  RooFormulaVar ecor2("ecor2","","1./(@0)*@1",RooArgList(*tgtvar2,*sigmeanlim2));
  RooRealVar *ecorvar2 = (RooRealVar*)hdata2->addColumn(ecor2);
  ecorvar2->setRange(0.,2.);
  ecorvar2->setBins(800);
  
  //formula for raw energy/true energy (1.0/(etrue/eraw))
  RooFormulaVar raw2("raw2","","1./@0",RooArgList(*tgtvar2));
  RooRealVar *rawvar2 = (RooRealVar*)hdata2->addColumn(raw2);
...
//heold->GetXaxis()->SetRangeUser(0.6,1.2);
  ...
  cout<<"center of Eraw/Etrue: "<<heraw->GetBinCenter(heraw->GetMaximumBin())<<endl;
...
*/
  ...
  leg->AddEntry(hecor,Form("E_{cor}/E_{true}, old cut, #sigma_{eff}=%4.3f, FWHM=%4.3f", effsigma_cor, fwhm_cor),"l");
  leg->AddEntry(hecor2,Form("E_{cor}/E_{true}, new cut, #sigma_{eff}=%4.3f, FWHM=%4.3f", effsigma_cor2, fwhm_cor2),"l");
  leg->AddEntry(heraw,Form("E_{raw}/E_{true}, old cut, #sigma_{eff}=%4.3f, FWHM=%4.3f", effsigma_raw, fwhm_raw),"l");
  leg->AddEntry(heraw2,Form("E_{raw}/E_{true}, new cut, #sigma_{eff}=%4.3f, FWHM=%4.3f", effsigma_raw2, fwhm_raw2),"l");
  leg->SetFillStyle(0);
  leg->SetBorderSize(0);
