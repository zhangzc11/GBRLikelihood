...
void eregtraining_13TeV_Pi0_Non20(bool dobarrel=true, bool doele=false, int gammaID=0, int Non20=1) {
  //gammaID = 0(both gamma), 1(gamma1), 2(gamma2) 
  //output dir
  //TString dirname = "/data/bendavid/eregexampletest/"; 
  ostringstream convert_dirname;
  convert_dirname<<"ereg_ws_Non20_"<<Non20<<"/bothGammas/";


  std::string dirname = convert_dirname.str();;//"ereg_ws/bothGammas/";
  if(gammaID == 1)
  {
...
//    tree->Add("/data/bendavid/regTreesAug1/hgg-2013Final8TeV_reg_s12-pj40-2em-v7n_noskim.root");    
    ...
    {
    tree = new TChain("Tree_Optim_gamma1");
    }
    else if(gammaID==2)
    {
    ...
    if(dobarrel)
    {
    tree->Add("/afs/cern.ch/work/z/zhicaiz/public/ECALpro_MC_TreeForRegression/sum_Pi0Gun_NoPU_EB_combine.root");      
    }
    else
     {
    tree->Add("/afs/cern.ch/work/z/zhicaiz/public/ECALpro_MC_TreeForRegression/sum_Pi0Gun_NoPU_EE_combine.root");      
     }
	cout<<"DEBUG 006..."<<endl; 
...
/*
  ...
  */
  ...
  TCut Events_1on20 = "(Entry$%40>37)";//38,39
  TCut Events_2on20 = "(Entry$%40>35)";//36-39
  TCut Events_3on20 = "(Entry$%40>33)";//38,39
  TCut Events_4on20 = "(Entry$%40>31)";//38,39
  TCut Events_5on20 = "(Entry$%40>29)";//38,39
  TCut Events_6on20 = "(Entry$%40>27)";//38,39
  TCut Events_7on20 = "(Entry$%40>25)";//38,39
  TCut Events_8on20 = "(Entry$%40>23)";//38,39
  TCut Events_9on20 = "(Entry$%40>21)";//38,39
  TCut Events_10on20 = "(Entry$%40>19)";//38,39
  TCut Events_11on20 = "(Entry$%40>17)";//38,39
  TCut Events_12on20 = "(Entry$%40>15)";//38,39
  TCut Events_13on20 = "(Entry$%40>13)";//38,39
  TCut Events_14on20 = "(Entry$%40>11)";//38,39
  TCut Events_15on20 = "(Entry$%40>9)";//38,39
  TCut Events_16on20 = "(Entry$%40>7)";//38,39
  TCut Events_17on20 = "(Entry$%40>5)";//38,39
  TCut Events_18on20 = "(Entry$%40>3)";//38,39
  TCut Events_19on20 = "(Entry$%40>1)";//38,39
  TCut Events_1on20_test = "(Entry$%40<2)";//0-1
  TCut Events_2on20_test = "(Entry$%40<4)";//0-3
  TCut Events_3on20_test = "(Entry$%40<6)";//0-5
	
  int int_Non20 = 39-2*Non20;
  ostringstream convert;
  convert<<"(Entry$%40>"<<int_Non20<<")";
 
  std::string s_on20 = convert.str();//to_string(39-2*Non20);
  TCut t_Non20 = TCut(s_on20.c_str());
  cout<<"the cut is:   "<<t_Non20<<endl;
  weightvar.SetTitle(t_Non20*selcut);
