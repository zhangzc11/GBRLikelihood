...
void eregtesting_13TeV_Pi0_oldDist(bool dobarrel=true, bool doele=false,int gammaID=0) {
  ...
  TString infile = TString::Format("../../Spring16TrainSpring15Test/ereg_ws_newCut/%s/%s",gammaDir.Data(),fname.Data());
  ...
  if (doele) {
    ...
    if(dobarrel)
    {
    TFile *fdin = TFile::Open("/afs/cern.ch/work/z/zhicaiz/public/ECALpro_MC_TreeForRegression/ND/PU_Spring15_EB_combine_test.root");
  ...
  //TCut selcut = "(STr2_enG_nocor/cosh(STr2_Eta)>1.0) && (STr2_S4S9 > 0.75) && (STr2_isMerging < 2) && (STr2_DeltaR < 0.03) && (STr2_iEta_on2520==0 || STr2_iPhi_on20==0) ";
